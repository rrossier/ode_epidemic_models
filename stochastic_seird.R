...
seird_generator <- odin::odin({
  ...
  beta <- user(0.4) # infection rate
  delta <- user(0.2) # inverse incubation period
  gamma_R <- user(0.9) # recovery rate
  gamma_D <- user(0.0001) # death rate
  mu <- user(0.035) # CFR: deaths/infected
  omega <- user(0.9) # rate of waning immunity
